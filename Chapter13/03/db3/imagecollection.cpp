...
void ImageCollection::populateDatabase()
{
    ...
    qry.prepare("CREATE TABLE IF NOT EXISTS images (id INTEGER PRIMARY KEY,data BLOB)");

    if(!qry.exec())
        qFatal("fail: can not create table images");

    qry.prepare("CREATE TABLE IF NOT EXISTS tags (id INTEGER,tag VARCHAR(30))");

    if(!qry.exec())
        qFatal("fail: can not create table tags");
}
...
QStringList ImageCollection::getTags()
{
    ...
    if(!qry.exec())
        qFatal("fail: can not select tags");
